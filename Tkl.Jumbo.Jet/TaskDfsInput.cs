...
namespace Tkl.Jumbo.Jet
{
    ...
    public class TaskDfsInput : ICloneable
    {
        ...
        public IRecordReader CreateRecordReader(DfsClient dfsClient, TaskExecutionUtility taskExecution)
        {
            if( dfsClient == null )
                throw new ArgumentNullException("dfsClient");
            Type recordReaderType = RecordReaderType;
            DfsInputStream inputStream = dfsClient.OpenFile(Path);
            long offset;
            long size;
            long blockSize = inputStream.BlockSize;
            offset = blockSize * (long)Block;
