...
namespace Tkl.Jumbo.Jet.Jobs
{
    ...
    /// </summary>
    public sealed class OldJobBuilder
    {
        #region Nested types

        private sealed class RecordReaderReference<T> : RecordReader<T>
        {
            public RecordReaderReference(OldJobBuilder jobBuilder, string input, Type recordReaderType)
            {
            ...
            public OldJobBuilder JobBuilder { get; private set; }
       ...
       {
            public RecordWriterReference(OldJobBuilder jobBuilder, string output, Type recordWriterType, int blockSize, int replicationFactor)
            {
                JobBuilder = jobBuilder;
                Output = output;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OldJobBuilder"/> class.
        /// </summary>
        public OldJobBuilder()
            : this(null, null)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="OldJobBuilder"/> class with the specified DFS and Jet clients.
        /// </summary>
        ...
        public OldJobBuilder(DfsClient dfsClient, JetClient jetClient)
        {
        ...
        public RecordWriter<T> CreateRecordWriter<T>(string output, Type recordWriterType, int blockSize, int replicationFactor)
        {
            ...
            if( recordWriterType.IsGenericTypeDefinition )
                recordWriterType = recordWriterType.MakeGenericType(typeof(T));
        ...
        /// <param name="task">The task function.</param>
        /// <param name="stageSettings">A dictionary containing settings to use for the stage. May be <see langword="null"/>.</param>
        public void ProcessRecords<TInput, TOutput>(RecordReader<TInput> input, RecordWriter<TOutput> output, TaskFunctionWithContext<TInput, TOutput> task, IDictionary<string, string> stageSettings)
         {
            if( input == null )
                throw new ArgumentNullException("input");
            if( output == null )
            ...
            where TKey : IComparable<TKey>
        {
            ...
            if( collector == null )
            {
                ...
                RecordCollector<Pair<TKey, TValue>> intermediateCollector = new RecordCollector<Pair<TKey, TValue>>() { PartitionCount = 1 };
            ...
            else
            {
                if( collector.InputStage != null )
                {
                    ...
                    if( !(collector.ChannelType == ChannelType.Pipeline || (collector.InputStage.TaskCount == 1 && (collector.PartitionCount <= 1 || output is RecordWriterReference<Pair<TKey, TValue>>))) )
                    {
                        // We'll need a second step, so create an intermedate collector and modify the original input channel
                        intermediateCollector = new RecordCollector<Pair<TKey, TValue>>()
                            {
            ...
            where TResult : new()
        {
            ...
            RecordCollector<TOuter> outerSortCollector = new RecordCollector<TOuter>()
                {
                    PartitionerType = outerInputCollector == null ? null : outerInputCollector.PartitionerType,
                    PartitionCount = outerInputCollector == null ? 1 : outerInputCollector.PartitionCount
                };
            RecordCollector<TInner> innerSortCollector = new RecordCollector<TInner>()
                {
                    PartitionerType = innerInputCollector == null ? null : innerInputCollector.PartitionerType,
                    PartitionCount = innerInputCollector == null ? 1 : innerInputCollector.PartitionCount
                };
            ...
            if( outputCollector == null || !((outputCollector.PartitionCount == 0 || outputCollector.PartitionCount == outerSortCollector.InputStage.InternalPartitionCount) && outputCollector.PartitionerType == null) )
            {
                ...
                outputCollector.SetPartitionCount(outerSortCollector.InputStage.InternalPartitionCount);
        ...
        private void ProcessRecordsInternal<TInput, TOutput>(RecordReader<TInput> input, RecordWriter<TOutput> output, Type taskType, string stageId, IDictionary<string, string> stageSettings, int noInputTaskCount)
        {
            ...
            if( outputRef != null )
            {
                ...
                AddAssemblies(outputWriterType.Assembly);
            }
            else
            {
                ...
                if( collector != null )
                {
                    ...
                    if( collector.PartitionerType != null )
                        AddAssemblies(collector.PartitionerType.Assembly);
                }
                else
            ...
            if( inputRef != null )
            {
                ...
                AddAssemblies(inputRef.RecordReaderType.Assembly);
            }
            else if( input != null )
            {
        ...
        private void SortRecordsInternal<T>(RecordReader<T> input, RecordWriter<T> output, Type comparerType, bool forceNoMergeStage)
        {
            ...
            if( inputCollector == null )
            {
                ...
                else
                {
                    ...
                    RecordCollector<T> intermediateCollector = new RecordCollector<T>() { PartitionCount = 1 };
            ...
            else
            {
                if( inputCollector.InputStage != null )
                {
                    ...
                    {
                        intermediateCollector = new RecordCollector<T>()
                        {
        ...
        private void AddAssemblies(Assembly assembly)
        {
