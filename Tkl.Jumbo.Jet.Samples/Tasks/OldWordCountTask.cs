...
namespace Tkl.Jumbo.Jet.Samples.Tasks
{
    ...
    /// </summary>
    [AllowRecordReuse]
    public class OldWordCountTask : IPullTask<StringWritable, KeyValuePairWritable<StringWritable, Int32Writable>>
    {
        #region Nested types
        ...
        public void Run(RecordReader<StringWritable> input, RecordWriter<KeyValuePairWritable<StringWritable, Int32Writable>> output)
        {
            ...
            record.Key = new StringWritable();
            record.Value = new Int32Writable();
            foreach( var item in wordCount )
            {
                record.Key.Value = item.Key;
                record.Value.Value = item.Value.Count;
