...
function Invoke-Server($HostName, [string]$Command)
{
    ...
    {
        Invoke-Command -ComputerName $HostName -ScriptBlock { &"$Using:scriptDir\Run-Server.ps1" $Using:StartStop $Using:Command } |% { "$($_.PSComputerName): $_" }
    }
}
...
{
    Invoke-Server $JUMBO_JOBSERVER JobServer
    Invoke-Server $JUMBO_JOBSERVER JetWeb
}

$HostNames = Get-Content groups |? { $_ -ne "masters" } |% { Get-Content $_ }
Invoke-Server $HostNames TaskServer
